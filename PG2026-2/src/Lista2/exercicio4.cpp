	...
	while (!glfwWindowShouldClose(window))
	{
		// Este trecho de código é totalmente opcional: calcula e mostra a contagem do FPS na barra de título
		{
			...
			if (title_countdown_s <= 0.0 && elapsed_s > 0.0)
			{
				...
				char tmp[256];
				snprintf(tmp, sizeof(tmp), "Ola Lista 2, Ex 4! -- Rossana\tFPS %.2lf", fps);
...
int setupGeometry()
{
	...
	// Triangulo deslocado para o quadrante SUPERIOR DIREITO da janela.
	// Esse quadrante ocupa x de 400 a 800 e y de 0 a 300 (lembre: y cresce para baixo,
	// logo y pequeno = topo da tela). O centro dele fica em (600, 150).
	// Translacao aplicada em relacao ao centro da janela: +200 em x, -150 em y.
	GLfloat vertices[] = {
		// x     y      z
		550.0, 200.0, 0.0,   // v0
		650.0, 200.0, 0.0,   // v1
